...
| Liveness — UDP+IPsec idle dereg | Implemented | `registrar.liveness.{enabled,keepalive_interval_secs,idle_multiplier,probe_timeout_ms}` | Detects a dead UE on the production Gm without a SIP de-REGISTER. The 30 s sweep polls kernel XFRM SA inbound use-time (one `XFRM_MSG_GETSA` netlink dump — no per-packet hot-path cost; the UE's RFC 6223 keepalive keeps the SA warm). A binding silent beyond `idle_multiplier × keepalive_interval` is probed with one OPTIONS over the captured flow; no answer → deregister. SA teardown (`sweep_expired`) also drops the matching binding. EPC-independent backstop for SMF crash / PCRF-no-Rx-ASR / silent radio loss. Idle-probe path needs kernel XFRM + lab validation. Default off. |
