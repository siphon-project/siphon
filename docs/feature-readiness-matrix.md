...
| Diameter serve-on-outbound (HSS→DRA) | Implemented | `diameter.tenants.<t>.connect_to` | A server NF that **initiates** the connection (e.g. an HSS dialling a DRA) but **answers** the requests relayed back over it. siphon sends the CER (tenant identity), then routes inbound requests to `@diameter.on_request` exactly like the listener path — transport direction is independent of request direction (RFC 6733 §2.1). Works without `diameter.listen`. TCP + SCTP. |
...
| Diameter Routing Agent (server mode) | Implemented | `diameter.listen`, `diameter.tenants`, `@diameter.on_inbound_cer`, `@diameter.on_request`, `@diameter.on_request_completed` | Accepts inbound Diameter (TCP + SCTP), two Rust-only auth gates (source-IP CIDR ACL + Origin-Host validation, both before any Python), per-tenant CEA identity, lossless AVP tree (`DiameterRequest`/`DiameterAnswer` get/set/remove/insert/iter), `req.forward_to(peer)` relay with Route-Record loop detection (3005) + per-call timeout, `diameter.peer_pool(tenant, target)` (round-robin / weighted / sticky over state-as-truth liveness), `diameter.config` snapshot (no YAML hot-reload — restart for route changes), `diameter.event_sink` (file/none; clickhouse/kafka feature-gated). `None`→3002. Inbound **and** outbound TCP+SCTP (`peer::connect_with_transport`). The ClickHouse/Kafka sinks are follow-ups. See `examples/dra.{py,yaml}`. |
